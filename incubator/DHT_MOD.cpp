...
#endif
  _maxcycles =			//MODIFIED BY F3RCODE attending to datasheet timings
      microsecondsToClockCycles(80); // 80 us timeout for
...
float DHT::readHumidityAlreadyRead() {
  float f = NAN;
...
bool DHT::read(bool force) {
  ...
  case DHT22:
  case DHT21:
    delayMicroseconds(600); // data sheet says "at least 500us" MODIFIED BY F3RCODE (AS DATASHEET CONVEYS)
    break;
  case DHT11:
  default:
  ...
  cli();//MODIFIED BY F3RCODE (critical zone)
   ...
   sei();//MODIFIED BY F3RCODE (ends critical zone)
